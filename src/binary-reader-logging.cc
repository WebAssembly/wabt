...
Result BinaryReaderLogging::OnEvent(Index index, Index kind, Index sig_index) {
  LOGF("OnEvent(index: %" PRIindex ", sig_index: %" PRIindex "\n", index,
       sig_index);
  return reader_->OnEvent(index, kind, sig_index);
}
