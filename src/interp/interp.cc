...
static void InitFromData(u32 log2_size,
                         Array* dst,
                         u32 dst_offset,
                         u32 size,
                         const u8* src) {
...
RunResult Thread::StepInternal(Trap::Ptr* out_trap) {
  ...
  switch (instr.op) {
    ...
    case O::ArrayNewData: {
      ...
      InitFromData(log2_size, array.get(), 0, size, data.desc().data.data() + offset);
      Push(array->self());
      break;
    }
    ...
    case O::ArrayNewElem: {
      ...
      for (size_t i = 0; i < array_ptr->Size(); i++) {
        array_ptr->SetItem(i, Value::Make(segment.elements()[offset + i]));
      }
      Push(array->self());
      break;
    }
