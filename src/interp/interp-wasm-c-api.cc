...
enum class WasmRefType { Extern, Module, Instance, Trap, Host };
...
struct wasm_foreign_ref_t : wasm_ref_t {
  wasm_foreign_ref_t() : wasm_ref_t(WasmRefType::Host) {}
...
RefType ToRefType(ExternalKind kind) {
  switch (kind) {
    case ExternalKind::Func:
      return RefType::Func;
    case ExternalKind::Global:
      return RefType::Global;
    case ExternalKind::Table:
      return RefType::Table;
    case ExternalKind::Memory:
      return RefType::Memory;
...
static TypedValue ToWabtValue(const wasm_val_t& value) {
  ...
  switch (value.kind) {
    ...
    case WASM_FUNCREF: {
      ...
      switch (ref->kind) {
        case WasmRefType::Extern: {
          auto* ext = static_cast<wasm_extern_t*>(ref);
          out.set_ref(Ref{ToRefType(ext->kind), ext->index});
          break;
        }
        case WasmRefType::Host: {
          // out.set_ref(Ref{RefType::Host, {.host_ptr = ref}});
          break;
        }
        case WasmRefType::Module:
        case WasmRefType::Instance:
...
bool wasm_table_grow(wasm_table_t* t,
  ...
  Ref init_ref{RefType::Func, kInvalidIndex};
  if (ext) {
