...
static void print_error(WasmLocation* loc, const char* fmt, ...) {
  va_list args;
  va_start(args, fmt);
  if (loc)
...
static int check_duplicate_bindings(WasmBindingVector* bindings,
                                    const char* desc) {
  int i, j;
  int result = WASM_OK;
  for (i = 1; i < bindings->size; ++i) {
    ...
    for (j = 0; j < i; ++j) {
      ...
      if (string_slices_are_equal(&ib->name, &jb->name)) {
        print_error(&ib->loc, "redefinition of %s \"%.*s\"", desc,
...
static int check_var(WasmBindingVector* bindings,
                     int max_index,
                     WasmVar* var,
                     const char* desc,
                     int* out_index) {
  ...
  if (var->type == WASM_VAR_TYPE_NAME)
    print_error(&var->loc, "undefined %s variable \"%.*s\"", desc,
                var->name.length, var->name.start);
  else
    print_error(&var->loc, "%s variable out of range (max %d)", desc,
                max_index);
  return WASM_ERROR;
}

static int check_func_var(WasmModule* module,
                          WasmVar* var,
                          WasmFunc** out_func) {
  int index;
  if (check_var(&module->func_bindings, module->funcs.size, var, "function",
                &index) != WASM_OK)
    return WASM_ERROR;
...
static int check_import_var(WasmModule* module,
                            WasmVar* var,
                            WasmImport** out_import) {
  int index;
  if (check_var(&module->import_bindings, module->imports.size, var, "import",
                &index) != WASM_OK)
    return WASM_ERROR;
...
static int check_func_type_var(WasmModule* module,
                               WasmVar* var,
                               WasmFuncType** out_func_type) {
  int index;
  if (check_var(&module->func_type_bindings, module->func_types.size, var,
...
static int check_local_var(WasmFunc* func,
                           WasmVar* var,
                           WasmType* out_type) {
  int index;
  if (check_var(&func->params_and_locals.bindings,
...
static int check_global_var(WasmModule* module,
    WasmVar* var, WasmType* out_type) {
  int index;
  if (check_var(&module->globals.bindings, module->globals.types.size, var,
...
static int check_type(WasmLocation* loc,
                      WasmType actual,
                      WasmType expected,
                      const char* desc) {
  if (expected != WASM_TYPE_VOID && (actual & expected) == 0) {
    print_error(loc, "type mismatch%s. got %s, expected %s", desc,
...
static int check_type_exact(WasmLocation* loc,
                            WasmType actual,
                            WasmType expected,
                            const char* desc) {
  if (actual != expected) {
...
static int check_type_arg_exact(WasmLocation* loc,
                                WasmType actual,
                                WasmType expected,
                                const char* desc,
                                int arg_index) {
  if (actual != expected) {
    print_error(loc, "type mismatch for argument %d of %s. got %s, expected %s",
...
static int check_label_var(WasmLabelNode* top_label,
                           WasmVar* var,
  ...
  if (var->type == WASM_VAR_TYPE_NAME)
    print_error(&var->loc, "undefined label variable \"%.*s\"",
                var->name.length, var->name.start);
  else
    print_error(&var->loc, "label variable out of range (max %d)",
...
static int push_label(WasmCheckContext* ctx,
  ...
  if (label && find_label_by_name(ctx->top_label, label)) {
    print_error(loc, "redefinition of label \"%.*s\"", label->length,
                label->start);
    /* add it anyway */
    result = WASM_ERROR;
  }
...
static int check_call(WasmCheckContext* ctx,
  ...
  if (expected_args == actual_args) {
    result |= check_type(loc, result_type, expected_type, "");
    char buffer[100];
    int i;
    ...
    print_error(loc, "too %s parameters to function. got %d, expected %d",
...
static int check_target(WasmCheckContext* ctx,
  ...
  switch (target->type) {
    case WASM_TARGET_TYPE_CASE:
      return check_var(bindings, num_cases, &target->var, "case", NULL);
    case WASM_TARGET_TYPE_BR:
      return check_label_var(ctx->top_label, &target->var, NULL);
    default:
      return WASM_ERROR;
  }
}
...
static int check_expr(WasmCheckContext* ctx,
  ...
  switch (expr->type) {
    case WASM_EXPR_TYPE_BINARY: {
      ...
      result |= check_type(&expr->loc, type, expected_type, desc);
    ...
    case WASM_EXPR_TYPE_BR: {
      WasmLabelNode* node;
      if (check_label_var(ctx->top_label, &expr->br.var, &node) == WASM_OK) {
        if (expr->br.expr) {
          ...
          result |= check_type(&expr->loc, WASM_TYPE_VOID, node->expected_type,
                               buffer);
        }
      } else {
        result = WASM_ERROR;
    ...
    case WASM_EXPR_TYPE_BR_IF:
      result |= check_expr(ctx, module, func, expr->br_if.cond, WASM_TYPE_I32,
                           " of br_if condition");
      result |= check_label_var(ctx->top_label, &expr->br_if.var, NULL);
      break;
    case WASM_EXPR_TYPE_CALL: {
      WasmFunc* func;
      if (check_func_var(module, &expr->call.var, &func) == WASM_OK) {
    ...
    case WASM_EXPR_TYPE_CALL_IMPORT: {
      WasmImport* import;
      if (check_import_var(module, &expr->call.var, &import) == WASM_OK) {
        WasmFuncSignature* sig = NULL;
        if (import->import_type == WASM_IMPORT_HAS_TYPE) {
          WasmFuncType* func_type;
          if (check_func_type_var(module, &import->type_var, &func_type) ==
              WASM_OK) {
            sig = &func_type->sig;
          } else {
            result = WASM_ERROR;
    ...
    case WASM_EXPR_TYPE_CALL_INDIRECT: {
      ...
      WasmFuncType* func_type;
      if (check_func_type_var(module, &expr->call_indirect.var, &func_type) ==
          WASM_OK) {
    ...
    case WASM_EXPR_TYPE_CONST:
      result |=
          check_type(&expr->loc, expr->const_.type, expected_type, desc);
      break;
    case WASM_EXPR_TYPE_CONVERT: {
      result |=
          check_type(&expr->loc, expr->convert.op.type, expected_type, desc);
    ...
    case WASM_EXPR_TYPE_GET_LOCAL: {
      WasmType type;
      if (check_local_var(func, &expr->get_local.var, &type) == WASM_OK)
        result |= check_type(&expr->loc, type, expected_type, desc);
      else
        result = WASM_ERROR;
      break;
    ...
    case WASM_EXPR_TYPE_HAS_FEATURE:
      result |= check_type(&expr->loc, WASM_TYPE_I32, expected_type,
                           " in has_feature");
      break;
    case WASM_EXPR_TYPE_IF:
      ...
      result |= check_expr(ctx, module, func, expr->if_.true_, expected_type,
                           " of if branch");
      result |= check_type(&expr->loc, WASM_TYPE_VOID, expected_type, desc);
      break;
    ...
    case WASM_EXPR_TYPE_LOAD_GLOBAL: {
      WasmType type;
      if (check_global_var(module, &expr->load_global.var, &type) == WASM_OK)
        result |= check_type(&expr->loc, type, expected_type, desc);
      else
        result = WASM_ERROR;
      break;
    ...
    case WASM_EXPR_TYPE_NOP:
      result |=
          check_type(&expr->loc, WASM_TYPE_VOID, expected_type, " in nop");
      break;
    case WASM_EXPR_TYPE_PAGE_SIZE:
      result |=
          check_type(&expr->loc, WASM_TYPE_I32, expected_type, " in page_size");
      break;
    case WASM_EXPR_TYPE_RETURN:
      if (expr->return_.expr) {
        ...
        result |=
            check_type(&expr->loc, WASM_TYPE_VOID, func->result_type, desc);
      }
      break;
    case WASM_EXPR_TYPE_SELECT:
      result |= check_type(&expr->loc, expr->select.type, expected_type, desc);
    ...
    case WASM_EXPR_TYPE_SET_LOCAL: {
      WasmType type;
      if (check_local_var(func, &expr->set_local.var, &type) == WASM_OK) {
        result |= check_type(&expr->loc, type, expected_type, desc);
    ...
    case WASM_EXPR_TYPE_STORE_GLOBAL: {
      WasmType type;
      if (check_global_var(module, &expr->store_global.var, &type) == WASM_OK) {
        result |= check_type(&expr->loc, type, expected_type, desc);
    ...
    case WASM_EXPR_TYPE_TABLESWITCH: {
      result |=
          check_duplicate_bindings(&expr->tableswitch.case_bindings, "case");
...
static int check_func(WasmCheckContext* ctx,
  ...
  if (func->flags & WASM_FUNC_FLAG_HAS_FUNC_TYPE) {
    WasmFuncType* func_type;
    if (check_func_type_var(module, &func->type_var, &func_type) == WASM_OK) {
      if (func->flags & WASM_FUNC_FLAG_HAS_SIGNATURE) {
        result |= check_type_exact(&func->loc, func->result_type,
        ...
        if (func->params.types.size == func_type->sig.param_types.size) {
          ...
          for (i = 0; i < func->params.types.size; ++i) {
            result |= check_type_arg_exact(
                &func->loc, func->params.types.data[i],
                func_type->sig.param_types.data[i], "function", i);
          }
        } else {
          print_error(&func->loc, "expected %d parameters, got %d",
  ...
  result |= check_duplicate_bindings(&func->params.bindings, "parameter");
  result |= check_duplicate_bindings(&func->locals.bindings, "local");
...
static int check_import(WasmCheckContext* ctx,
  ...
  return check_import_var(module, &import->type_var, NULL);
}

static int check_export(WasmCheckContext* ctx,
                        WasmModule* module,
                        WasmExport* export) {
  return check_func_var(module, &export->var, NULL);
}
...
static int check_table(WasmCheckContext* ctx,
  ...
  for (i = 0; i < table->size; ++i)
    result |= check_func_var(module, &table->data[i], NULL);
  return result;
}
...
static int check_module(WasmCheckContext* ctx, WasmModule* module) {
  ...
  for (i = 0; i < module->fields.size; ++i) {
    ...
    switch (field->type) {
      ...
      case WASM_MODULE_FIELD_TYPE_TABLE:
        if (seen_table) {
          print_error(&field->loc, "only one table allowed");
          result |= WASM_ERROR;
        }
      ...
      case WASM_MODULE_FIELD_TYPE_MEMORY:
        if (seen_memory) {
          print_error(&field->loc, "only one memory block allowed");
          result |= WASM_ERROR;
        }
  ...
  result |= check_duplicate_bindings(&module->func_bindings, "function");
  result |= check_duplicate_bindings(&module->import_bindings, "import");
  result |= check_duplicate_bindings(&module->export_bindings, "export");
  result |=
      check_duplicate_bindings(&module->func_type_bindings, "function type");
  result |= check_duplicate_bindings(&module->globals.bindings, "global");
...
static int check_invoke(WasmCheckContext* ctx,
  ...
  if (!ctx->last_module) {
    print_error(&invoke->loc, "invoke must occur after a module definition");
    return WASM_ERROR;
  }
  ...
  if (!export) {
    print_error(&invoke->loc, "unknown function export \"%.*s\"",
  ...
  int result =
      check_type(&invoke->loc, func->result_type, return_type, "");
  int i;
  for (i = 0; i < actual_args; ++i) {
    WasmConst* const_ = &invoke->args.data[i];
    result |= check_type_arg_exact(&const_->loc, const_->type,
  ...
  switch (command->type) {
    ...
    case WASM_COMMAND_TYPE_ASSERT_INVALID: {
      ...
      if (module_ok == WASM_OK) {
        ...
        print_error(&command->assert_invalid.module.loc,
