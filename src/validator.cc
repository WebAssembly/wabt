...
static Result CheckType(Type actual, Type expected) {
  ...
  switch (expected_type) {
    ...
    case Type::ExnRef:
      if (actual_type == Type::NullExnRef || actual_type == Type::ExnRef) {
        break;
      }
      return Result::Error;
    case Type::NullFuncRef:
    case Type::FuncRef:
      if (actual_type == Type::NullFuncRef || actual_type == Type::FuncRef) {
        break;
      }
      return Result::Error;
    case Type::NullExternRef:
    case Type::ExternRef:
      if (actual_type == Type::NullExternRef ||
          actual_type == Type::ExternRef) {
        break;
      }
      return Result::Error;
    case Type::NullRef:
    case Type::AnyRef:
      if (actual_type == Type::EqRef || actual_type == Type::NullRef ||
          actual_type == Type::AnyRef) {
        break;
      }
      [[fallthrough]];
    case Type::EqRef:
  ...
  for (const ModuleField& field : module->fields) {
    if (auto* f = dyn_cast<TypeModuleField>(&field)) {
      if (type_index == first_type_index) {
        ...
        result_ |= validator_.OnRecursiveGroup(range.first_type_index,
                                               range.type_count);
...
class ComponentValidator {
 ...
 private:
  ...
  static SharedComponentValidator::CoreFuncSignature GetSignature(
      FuncSignature& sig);
...
Result ComponentValidator::CheckComponent() {
  ...
  while (true) {
    while (i < def_list->Size()) {
      ...
      switch (section) {
        case ComponentSection::CoreModule: {
          ...
          CHECK_RESULT(validator_.OnCoreModule());
          for (auto& export_ : module->exports) {
            switch (export_->kind) {
              case ExternalKind::Func: {
                FuncSignature& sig =
                    module->funcs[export_->var.index()]->decl.sig;
                validator_.CoreModuleAddFunctionExport(export_->name,
                                                       GetSignature(sig));
                break;
              }
              case ExternalKind::Table:
                validator_.CoreModuleAddTableExport(export_->name);
                break;
              case ExternalKind::Memory: {
                bool is_64 =
                    module->memories[export_->var.index()]->page_limits.is_64;
                validator_.CoreModuleAddMemoryExport(export_->name, is_64);
                break;
              }
              case ExternalKind::Global:
                validator_.CoreModuleAddGlobalExport(export_->name);
                break;
              case ExternalKind::Tag:
                validator_.CoreModuleAddTagExport(export_->name);
                break;
            }
          }
          break;
        ...
        case ComponentSection::Component:
          ...
          i = 0;
          CHECK_RESULT(validator_.BeginComponent());
          continue;
    ...
    if (def_list->section() == ComponentSection::Component) {
      CHECK_RESULT(validator_.EndComponent());
...
SharedComponentValidator::CoreFuncSignature ComponentValidator::GetSignature(
    FuncSignature& sig) {
  if (sig.GetNumParams() == 4 && sig.GetNumResults() == 1) {
    if (sig.GetParamType(0) == Type::I32) {
      if (sig.GetParamType(1) == Type::I32 &&
          sig.GetParamType(2) == Type::I32 &&
          sig.GetParamType(3) == Type::I32 &&
          sig.GetResultType(0) == Type::I32) {
        return SharedComponentValidator::FunctionParamI32x4ResultI32;
      }
    } else if (sig.GetParamType(0) == Type::I64 &&
               sig.GetParamType(1) == Type::I64 &&
               sig.GetParamType(2) == Type::I64 &&
               sig.GetParamType(3) == Type::I64 &&
               sig.GetResultType(0) == Type::I64) {
      return SharedComponentValidator::FunctionParamI64x4ResultI64;
    }
  }
  return SharedComponentValidator::FunctionOther;
}
