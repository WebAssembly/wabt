...
Result WastParser::ParseRefType(Var* out_type) {
  ...
  // Validator must throw error for non-reference types.
  // Note: spec-test interpreter throws a syntax error.
  *out_type = Var(0, GetLocation());
  out_type->set_opt_type(type);
  return Result::Ok;
}
...
Result WastParser::ParseConstList(ConstVector* consts, ConstType type) {
  ...
  while (
      ...
      PeekMatchLpar(TokenType::RefExtern) || PeekMatchLpar(TokenType::RefI31) ||
