...
Result WastParser::ParseComponentIndex(ComponentSharedData* shared_data,
  ...
  *out_index = shared_data->TypeSize();
  auto value = std::make_unique<ComponentAliasOuter>(counter, index,
                                                     ComponentSort::Type);
...
Result WastParser::ParseComponentCanonOpts(
  ...
  while (true) {
    ...
    Consume();
    out_options->push_back(ComponentCanonOption{option, kInvalidIndex});
  }
}
  ...
  if (Peek(0) != TokenType::Lpar) {
    ...
    if (Peek(0) == TokenType::ValueType) {
      ...
      auto value = std::make_unique<ComponentValueType>(
          ComponentTypeDef::ValueType, type);
      shared_data->AppendType(std::move(value));
      return Result::Ok;
    }
  ...
  switch (token_type) {
    ...
    case TokenType::Variant: {
      ...
      auto type = std::make_unique<ComponentTypeItems>(
          ComponentTypeDef::Variant, &items);
      shared_data->AppendType(std::move(type));
      return Result::Ok;
    }
    case TokenType::List: {
      ...
      ComponentTypeDef type_value = ComponentTypeDef::List;
      ...
      if (PeekMatch(TokenType::Nat)) {
        ...
        type_value = ComponentTypeDef::ListFixed;
      }
    ...
    case TokenType::Tuple: {
      ...
      auto type = std::make_unique<ComponentTypeTuple>(&items);
      shared_data->AppendType(std::move(type));
      return Result::Ok;
    }
    case TokenType::Flags:
    case TokenType::Enum: {
      bool is_flags = token_type == TokenType::Flags;
      Consume();
      ...
      while (!PeekMatch(TokenType::Rpar)) {
        std::string id;
        CHECK_RESULT(ParseQuotedText(&id));
        items.emplace_back(string_table->Append(id));
      }
      ...
      auto type = std::make_unique<ComponentTypeNames>(type_value, &items);
      shared_data->AppendType(std::move(type));
      return Result::Ok;
    }
    ...
    case TokenType::Future: {
      ...
      ComponentType value_type;
      if (token_type == TokenType::Option || !PeekMatch(TokenType::Rpar)) {
        CHECK_RESULT(
            ParseComponentValType(shared_data, string_table, &value_type));
      }
      ...
      if (token_type == TokenType::Stream) {
        type_value = ComponentTypeDef::Stream;
      } else if (token_type == TokenType::Future) {
    ...
    case TokenType::Borrow: {
      ComponentTypeDef type_value = (token_type == TokenType::Own)
