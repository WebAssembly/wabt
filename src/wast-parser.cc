  ...
  switch (token.token_type()) {
    case TokenType::Record: {
      ...
      while (MatchLpar(TokenType::Field)) {
        ...
        items.emplace_back(ComponentTypeItems::Item{name, type});
        EXPECT(Rpar);
      }
    ...
    case TokenType::Variant: {
      ...
      auto type = std::make_unique<ComponentTypeItems>(
          ComponentTypeDef::Variant, &items, token.loc);
      shared_data->AppendType(std::move(type));
      return Result::Ok;
    }
    case TokenType::List: {
    ...
    case TokenType::Tuple: {
      ComponentTypeTuple::ItemVector items;
      ...
      auto type = std::make_unique<ComponentTypeTuple>(&items, token.loc);
      shared_data->AppendType(std::move(type));
      return Result::Ok;
    }
    ...
    case TokenType::Enum: {
      ...
      ComponentTypeNames::ItemVector items;
      ...
      auto type =
          std::make_unique<ComponentTypeNames>(type_value, &items, token.loc);
      shared_data->AppendType(std::move(type));
      return Result::Ok;
    }
