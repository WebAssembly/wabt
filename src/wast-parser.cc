...
Result WastParser::ParseValueTypeList(TypeVector* out_type_list,
  ...
  while (true) {
    ...
    if (type.is_index()) {
      if (Type::EnumIsReferenceWithIndex(type.opt_type())) {
...
void WastParser::VarToType(const Var& var, Type* type) {
  if (!Type::EnumIsReferenceWithIndex(var.opt_type())) {
    *type = var.to_type();
    return;
  }

  *type = Type(var.opt_type(), var.is_index() ? var.index() : kInvalidIndex);
