...
Result WastParser::ParseValueType(Var* out_type, bool is_field) {
  ...
  switch (type) {
    ...
    case Type::StructRef:
    case Type::ArrayRef:
      is_enabled = options_->features.gc_enabled();
      break;
    case Type::I8:
    case Type::I16:
      is_enabled = is_field;
...
Result WastParser::ParseTypeModuleField(Module* module) {
  ...
  if (options_->features.gc_enabled() && MatchLpar(TokenType::Sub)) {
    ...
    has_sub_type_var = ParseVarOpt(&sub_type_var, sub_type_var);
  }
  ...
  if (Match(TokenType::Func)) {
    ...
    if (has_sub_type_var) {
      func_type->gc_ext.sub_types.push_back(std::move(sub_type_var));
    }
    ...
    if (has_sub_type_var) {
      struct_type->gc_ext.sub_types.push_back(std::move(sub_type_var));
    }
    ...
    if (has_sub_type_var) {
      array_type->gc_ext.sub_types.push_back(std::move(sub_type_var));
    }
