...
static const size_t LEB_SEC_SIZE_GUESS = 1;
  ...
  switch (expr->type) {
    ...
    case WASM_EXPR_TYPE_CALL: {
      ...
      if (ctx->options->linkable) {
        add_reloc(ctx, WASM_RELOC_FUNC_INDEX);
        wasm_write_fixed_u32_leb128(&ctx->stream, index, "function index");
      } else {
    ...
    case WASM_EXPR_TYPE_GET_GLOBAL: {
      ...
      if (ctx->options->linkable) {
        add_reloc(ctx, WASM_RELOC_GLOBAL_INDEX);
...
static void write_reloc_section(Context* ctx, RelocSection* reloc_section) {
  ...
  begin_custom_section(ctx, section_name, LEB_SEC_SIZE_GUESS);
...
static void write_module(Context* ctx, const WasmModule* module) {
  ...
  if (module->func_types.size) {
    begin_known_section(ctx, WASM_BINARY_SECTION_TYPE, LEB_SEC_SIZE_GUESS);
  ...
  if (module->imports.size) {
    begin_known_section(ctx, WASM_BINARY_SECTION_IMPORT, LEB_SEC_SIZE_GUESS);
    ...
    for (i = 0; i < module->imports.size; ++i) {
      ...
      switch (import->kind) {
        case WASM_EXTERNAL_KIND_FUNC:
          if (ctx->options->linkable) {
            add_reloc(ctx, WASM_RELOC_TYPE_INDEX);
            wasm_write_fixed_u32_leb128(&ctx->stream, wasm_get_func_type_index_by_decl(
  ...
  if (num_funcs) {
    begin_known_section(ctx, WASM_BINARY_SECTION_FUNCTION, LEB_SEC_SIZE_GUESS);
  ...
  if (num_tables) {
    begin_known_section(ctx, WASM_BINARY_SECTION_TABLE, LEB_SEC_SIZE_GUESS);
  ...
  if (num_memories) {
    begin_known_section(ctx, WASM_BINARY_SECTION_MEMORY, LEB_SEC_SIZE_GUESS);
  ...
  if (num_globals) {
    begin_known_section(ctx, WASM_BINARY_SECTION_GLOBAL, LEB_SEC_SIZE_GUESS);
  ...
  if (module->exports.size) {
    begin_known_section(ctx, WASM_BINARY_SECTION_EXPORT, LEB_SEC_SIZE_GUESS);
  ...
  if (module->start) {
    ...
    if (start_func_index != -1) {
      begin_known_section(ctx, WASM_BINARY_SECTION_START, LEB_SEC_SIZE_GUESS);
  ...
  if (module->tables.size && module->elem_segments.size) {
    begin_known_section(ctx, WASM_BINARY_SECTION_ELEM, LEB_SEC_SIZE_GUESS);
  ...
  if (num_funcs) {
    begin_known_section(ctx, WASM_BINARY_SECTION_CODE, LEB_SEC_SIZE_GUESS);
    wasm_write_u32_leb128(&ctx->stream, num_funcs, "num functions");
  ...
  if (module->memories.size && module->data_segments.size) {
    begin_known_section(ctx, WASM_BINARY_SECTION_DATA, LEB_SEC_SIZE_GUESS);
  ...
  if (ctx->options->write_debug_names) {
    ...
    char desc[100];
    begin_custom_section(ctx, WASM_BINARY_SECTION_NAME, LEB_SEC_SIZE_GUESS);
