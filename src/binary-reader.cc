...
class BinaryReader {
 ...
 private:
  ...
  [[nodiscard]] Result ReadComponentCoreSort(
      ComponentBinaryCoreSort* out_core_sort);
  [[nodiscard]] Result ReadComponentSort(
      ComponentBinarySort* out_sort,
      ComponentBinaryCoreSort* out_core_sort);
...
Result BinaryReader::ReadComponentCoreSort(
    ComponentBinaryCoreSort* out_core_sort) {
  uint8_t sort_code;
  ...
  switch (static_cast<ComponentBinaryCoreSort>(sort_code)) {
    ...
    case ComponentBinaryCoreSort::Instance:
      *out_core_sort = static_cast<ComponentBinaryCoreSort>(sort_code);
      break;
    default:
...
Result BinaryReader::ReadComponentSort(ComponentBinarySort* out_sort,
                                       ComponentBinaryCoreSort* out_core_sort) {
  uint8_t sort_code;
  ...
  switch (static_cast<ComponentBinarySort>(sort_code)) {
    case ComponentBinarySort::Core:
      *out_sort = ComponentBinarySort::Core;
      return ReadComponentCoreSort(out_core_sort);
    ...
    case ComponentBinarySort::Instance:
      *out_sort = static_cast<ComponentBinarySort>(sort_code);
      *out_core_sort = ComponentBinaryCoreSort::NonCore;
      break;
    default:
...
Result BinaryReader::ReadComponentCoreInstance() {
  ...
  for (uint32_t i = 0; i < argument_count; i++) {
    std::string_view name;
    uint32_t index;
    ComponentBinaryCoreSort core_sort = ComponentBinaryCoreSort::Instance;

    CHECK_RESULT(ReadStr(&name, "name"));
    if (is_inline) {
      CHECK_RESULT(ReadComponentCoreSort(&core_sort));
    } else {
    ...
    CHECK_RESULT(ReadU32Leb128(&index, "index"));
    arguments.push_back(
        ComponentNamedSort{name, ComponentBinarySort::Core, core_sort, index});
  }
...
Result BinaryReader::ReadComponentInstance() {
  ...
  for (uint32_t i = 0; i < argument_count; i++) {
    ...
    CHECK_RESULT(ReadU32Leb128(&index, "index"));
    arguments.push_back(ComponentNamedSort{name, sort, core_sort, index});
  }
...
Result BinaryReader::ReadComponentInlineInstance() {
  ...
  for (uint32_t i = 0; i < argument_count; i++) {
    ...
    CHECK_RESULT(ReadComponentSort(&sort, &core_sort));
    CHECK_RESULT(ReadU32Leb128(&index, "index"));
    arguments.push_back(ComponentNamedExportInfo{
        name, version_suffix, sort, core_sort, has_version_suffix, index});
  }
  ...
  switch (static_cast<ComponentBinaryAlias>(target_type)) {
    ...
    case ComponentBinaryAlias::CoreExport: {
      ...
      if (static_cast<ComponentBinaryAlias>(target_type) ==
        ...
        COMPONENT_CALLBACK(OnAliasCoreExport, sort, core_sort, instance_index,
                           export_name);
      }
      break;
    }
...
Result BinaryReader::ReadComponentCanonOptions(
  ...
  while (option_count > 0) {
    ...
    ComponentBinaryCanonOption option =
        static_cast<ComponentBinaryCanonOption>(option_value);
    switch (option) {
      case ComponentBinaryCanonOption::StrEncUtf8:
      case ComponentBinaryCanonOption::StrEncUtf16:
      case ComponentBinaryCanonOption::StrEncLatin1Utf16:
      case ComponentBinaryCanonOption::Async:
        break;
      case ComponentBinaryCanonOption::Memory:
      case ComponentBinaryCanonOption::Realloc:
      case ComponentBinaryCanonOption::PostReturn:
      case ComponentBinaryCanonOption::Callback:
  ...
  if (type == ComponentBinaryType::Some) {
    ...
    switch (external_info->sort) {
      ...
      case ComponentBinarySort::Type:
        ...
        external_info->external =
            static_cast<ComponentBinaryExternal>(type_code);
        if (external_info->external == ComponentBinaryExternal::TypeSubRes) {
          read_index = false;
        } else if (external_info->external != ComponentBinaryExternal::TypeEq) {
          ERROR("Unknown core type: %" PRIx8, type_code);
          return Result::Error;
        }
        break;
  ...
  while (true) {
    while (state_.offset < read_end) {
      ...
      switch (static_cast<ComponentBinarySection>(section_code)) {
        case ComponentBinarySection::Custom: {
        ...
        case ComponentBinarySection::CoreInstance: {
          uint32_t count;
          CHECK_RESULT(ReadU32Leb128(&count, "count"));
        ...
        case ComponentBinarySection::Instance: {
          uint32_t count;
          CHECK_RESULT(ReadU32Leb128(&count, "count"));
        ...
        case ComponentBinarySection::Canon: {
          uint32_t count;
          CHECK_RESULT(ReadU32Leb128(&count, "count"));
