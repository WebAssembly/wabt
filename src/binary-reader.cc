...
Result BinaryReader::ReadComponentType() {
  ...
  while (true) {
    ...
    if (ComponentType::IsPrimitiveType(type_code)) {
      ...
      COMPONENT_CALLBACK(OnPrimitiveType, ComponentTypeLoc{type, loc});
    } else {
      ...
      switch (def_type) {
        ...
        case ComponentTypeDef::Tuple: {
          uint32_t type_count;
          std::vector<ComponentTypeLoc> types;
        ...
        case ComponentTypeDef::Enum: {
          uint32_t name_count;
          std::vector<ComponentStringLoc> names;
          CHECK_RESULT(ReadU32Leb128(&name_count, "name count"));
          names.reserve(name_count);

          while (names.size() < name_count) {
            std::string_view name;
            Location name_loc(state_.offset);
            CHECK_RESULT(ReadStr(&name, "name"));
            names.emplace_back(name, name_loc);
          }
        ...
        case ComponentTypeDef::ResourceAsync: {
          ...
          uint32_t res_type = 0;
          CHECK_RESULT(ReadS32Leb128(&res_type, "resource type"));
          if (res_type != static_cast<uint32_t>(Type::I32)) {
            ERROR("Unsupported resource type: %" PRIx8, type_code);
            break;
          }
          ...
          switch (static_cast<ComponentBinaryType>(type_code)) {
            case ComponentBinaryType::None:
              break;
            case ComponentBinaryType::Some:
              CHECK_RESULT(ReadComponentIndex(is_async ? &callback : &dtor,
                                              "dtor"));
              break;
            default:
          ...
          if (is_async) {
            COMPONENT_CALLBACK(OnResourceAsyncType, dtor, callback);
          } else {
            COMPONENT_CALLBACK(OnResourceType, dtor);
          }
          break;
        }
        default:
