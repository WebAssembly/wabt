...
#define DEFINE_ATOMIC_LOAD(name, t1, t2, t3, force_read)         \
  static inline t3 name(wasm_rt_memory_t* mem, u64 addr) {       \
    ...
    result = atomic_load_##t1(&LEADDR(mem, addr, t1));           \
    force_read(result);                                          \
    return (t3)(t2)result;                                       \
  }
...
#define DEFINE_ATOMIC_STORE(name, t1, t2)                              \
  static inline void name(wasm_rt_memory_t* mem, u64 addr, t2 value) { \
    ...
    ATOMIC_ALIGNMENT_CHECK(addr, t1);                                  \
    t1 wrapped = (t1)value;                                            \
    atomic_store_##t1(&LEADDR(mem, addr, t1), wrapped);                \
  }
...
#define DEFINE_ATOMIC_RMW(name, op, t1, t2)                          \
  static inline t2 name(wasm_rt_memory_t* mem, u64 addr, t2 value) { \
    ...
    ATOMIC_ALIGNMENT_CHECK(addr, t1);                                \
    t1 wrapped = (t1)value;                                          \
    t1 ret = atomic_##op##_##t1(&LEADDR(mem, addr, t1), wrapped);    \
    return (t2)ret;                                                  \
  }
...
#define DEFINE_ATOMIC_CMP_XCHG(name, t1, t2)                             \
  static inline t1 name(wasm_rt_memory_t* mem, u64 addr, t1 expected,    \
    ...
    t2 old = atomic_compare_exchange_##t2(                               \
        &LEADDR(mem, addr, t2), &expected_wrapped, replacement_wrapped); \
    return (t1)old;                                                      \
  }
