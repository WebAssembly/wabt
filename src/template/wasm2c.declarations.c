...
#if WABT_BIG_ENDIAN
#define LEADDR(mem, addr, t) mem->data[mem->size - (addr) - sizeof(t)]
#else
#define LEADDR(mem, addr, t) mem->data[addr]
#endif
...
#define DEFINE_LOAD(name, t1, t2, t3, force_read)                \
  static inline t3 name(wasm_rt_memory_t* mem, u64 addr) {       \
    ...
    t1 result;                                                   \
    wasm_rt_memcpy(&result, &LEADDR(mem, addr, t1), sizeof(t1)); \
    force_read(result);                                          \
    return (t3)(t2)result;                                       \
  }
...
#define DEFINE_STORE(name, t1, t2)                                     \
  static inline void name(wasm_rt_memory_t* mem, u64 addr, t2 value) { \
    MEMCHECK(mem, addr, t1);                                           \
    t1 wrapped = (t1)value;                                            \
    wasm_rt_memcpy(&LEADDR(mem, addr, t1), &wrapped, sizeof(t1));      \
  }
...
static inline void memory_fill(wasm_rt_memory_t* mem, u32 d, u32 val, u32 n) {
  RANGE_CHECK(mem, d, n);
#if WABT_BIG_ENDIAN
  memset(mem->data + (mem->size - d - n), val, n);
#else
...
#if WABT_BIG_ENDIAN
  memmove(dest->data + (dest->size - dest_addr - n),
          src->data + (src->size - src_addr - n), n);
#else
