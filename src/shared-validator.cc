...
Result SharedValidator::RecursionDetector::CheckRecursion(Type type,
                                                          const char* desc) {
  ...
  auto iter = validator->func_types_.find(index);

  if (iter == validator->func_types_.end()) {
    validator->PrintError(Location(),
                          "reference %" PRIindex " is out of range in %s",
                          index, desc);
    return Result::Error;
  }
  ...
  auto insert_result = processed_func_types.insert(std::pair(index, iteration));

  if (insert_result.second) {
    visited_func_types.push_back(index);
  } else if (insert_result.first->second == iteration) {
...
Result SharedValidator::EndTypeSection() {
  ...
  // Checking invalid references and circles
  RecursionDetector detector(this);

  for (auto func_type : func_types_) {
    auto insert_result = detector.processed_func_types.insert(
        std::pair(func_type.first, detector.iteration));

    if (insert_result.second) {
      detector.visited_func_types.push_back(func_type.first);

      do {
        FuncType& current_func_type =
            func_types_[detector.visited_func_types.back()];
        detector.visited_func_types.pop_back();

        for (auto type : current_func_type.params) {
          result |= detector.CheckRecursion(type, "params");
        }

        for (auto type : current_func_type.results) {
          result |= detector.CheckRecursion(type, "results");
        }
      } while (!detector.visited_func_types.empty());
    }

    detector.iteration++;
  }

  if (detector.recursion_found) {
    PrintError(Location(), "unknown type: circular declaration");
  }
