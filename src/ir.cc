...
Var::Var(Type type, const Location& loc)
  ...
  } else if (type.IsGenericRef()) {
    index_ = type.IsNullableGenericRef() ? Type::ReferenceOrNull
...
Type Var::to_type() const {
  if (opt_type_ == Type::FuncRef || opt_type_ == Type::ExternRef ||
      opt_type_ == Type::Reference || opt_type_ == Type::Ref ||
...
uint8_t ElemSegment::GetFlags(const Module* module) const {
  ...
  switch (kind) {
    case SegmentKind::Active: {
      ...
      if (elem_type.opt_type() != Type::FuncRef || table_index != 0) {
  ...
  bool all_ref_func =
      elem_type.opt_type() == Type::FuncRef &&
