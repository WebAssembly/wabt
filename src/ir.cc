...
Index Module::GetFuncTypeIndex(const FuncSignature& sig) const {
  ...
  // Recursive types with more than one type cannot be used as implicit type.
  while (range_index < recursive_ranges.size() &&
         recursive_ranges[range_index].type_count <= 1) {
    range_index++;
  }
