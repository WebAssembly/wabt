  ...
  while (true) {
    ...
    switch (type->type()) {
      ...
      case ComponentTypeDef::List:
        ...
        if (type->type() == ComponentTypeDef::ListFixed) {
      ...
      case ComponentTypeDef::Tuple: {
        ...
        WriteU32Leb128(stream_, size, "type count");
        for (const ComponentType& item_type : type->AsTypeTuple()->Items()) {
          WriteComponentValueType(item_type, false);
        }
        break;
      }
      ...
      case ComponentTypeDef::Enum: {
        uint32_t size =
            static_cast<uint32_t>(type->AsTypeNames()->Items().size());
        WriteU32Leb128(stream_, size, "type count");
        for (const std::string* name : type->AsTypeNames()->Items()) {
          WriteStr(stream_, *name, "name", PrintChars::Yes);
        }
        break;
      }
