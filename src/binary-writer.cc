  ...
  while (true) {
    ...
    switch (type->type()) {
      ...
      case ComponentTypeDef::Enum: {
        ...
        WriteU32Leb128(stream_, size, "type count");
        for (auto& item : type->AsTypeNames()->Items()) {
          WriteStr(stream_, *item.str, "name", PrintChars::Yes);
        }
        break;
      }
      ...
      case ComponentTypeDef::Resource:
        WriteType(stream_, Type::I32, "resource type");
        WriteComponentIndex(type->AsTypeIndex()->GetIndex(), "dtor");
        break;
      case ComponentTypeDef::ResourceAsync:
        WriteType(stream_, Type::I32, "resource type");
        WriteU32Leb128(stream_, type->AsTypeResourceAsync()->Dtor(), "dtor");
        WriteComponentIndex(type->AsTypeResourceAsync()->Callback(),
                            "callback");
        break;
