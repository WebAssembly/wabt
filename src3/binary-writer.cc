...
void BinaryWriter::WriteExpr(const Func* func, const Expr* expr) {
  switch (expr->type()) {
    ...
    case ExprType::BrOnCast: {
      ...
      Type type2 = var2.ParseRefType();
      Type type3 = var3.ParseRefType();
      if (type2.isRef()) {
        if (type3.isRefNull()) {
      ...
      switch (type->kind()) {
        ...
        case TypeEntryKind::Sub: {
          ...
          if (str == "") {
            ...
            int j = 0;
            if (moduletypesname.find(str) != moduletypesname.end()) {
              j = moduletypesname[str];
            }
